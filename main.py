...
# 1. Platform Brand Navigation Header
st.markdown("""
...
with col_display:
    ...
    st.markdown('</div>', unsafe_with_html=True)# Ingest underlying data structures
context = run_aquator_engine()
env = context.get("environment", {})
surface = env.get("surface_conditions", {})
layers = env.get("water_column_profile", {})
...
# Safely read the CSS file directly from the directory without triggering string validation errors
try:
    with open("style.css", "r") as f:
        st.markdown(f"<style>{f.read()}</style>", unsafe_with_html=True)
except Exception as e:
    # Safe fallback if the file isn't loaded yet
    st.warning("Custom stylesheet loading...")
